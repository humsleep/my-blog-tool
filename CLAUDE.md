...
**블로그 진단 (별도 진입점)**
- **내가 실제로 쓴 글이 노린 키워드**로 1페이지 진입율 측정(분야 자동 감지, Phase 59) + **최근 12편 본문 PostView.naver 측정**(글자수·이미지 정확도)
  - 노출 측정: 최근 글 제목→핵심 키워드 추출(`lib/diagnose/title-keyword.ts`) → 네이버 검색 → 내 글 랭크. 분야 고정 키워드가 아니라 내 글 기준이라 분야·규모 무관하게 공정
...
### 메뉴 구조 (확정 — Phase 58)
```
[Desktop Navbar]  키워드 리서치 ▼  글쓰기 ▼  진단  커뮤니티 ▼  이미지 ▼  연구실

키워드 리서치 ▼   글쓰기 ▼          커뮤니티 ▼          이미지 ▼
├ 인기검색어     ├ 빠른 시작       ├ 서이추 해요       ├ 이미지 검색
├ 키워드 분석    ├ 프롬프트 생성   ├ 정보 공유         └ 이미지 편집
└ 상위노출 분석  ├ AI 글쓰기       └ 체험단 동행해요
                 └ 에디터(발행)
※ 진단(/blog-diagnose), 연구실(/lab)은 평면 단일 링크

[Mobile Bottom Nav, md:hidden]
홈  도구  진단  커뮤니티      ※ 연구실은 햄버거 메뉴에 노출
```
