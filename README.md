...
운영 중인 네이버·티스토리 블로그를 "막연한 느낌"이 아니라 **측정 가능한 숫자**로 봅니다. 내가 실제로 쓴 글이 노린 키워드로 검색 1페이지 진입율을 재고(분야는 자동 감지), 최근 12편 본문을 직접 호출해 글자수·이미지 수까지 계산합니다.

| 블로거의 흔한 고민 | 이 도구의 답 |
|---|---|
| "내 블로그 점수가 막연해요" | 내 글이 노린 키워드 1페이지 진입율 → **0~100점 + 5단계 밴드** |
...
> 내가 쓴 글이 노린 키워드(분야 자동 감지) + 최근 본문 12편을 측정해 **0~100점**으로 평가합니다.
