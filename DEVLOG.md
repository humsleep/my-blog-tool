...
### ③ 대체 — AI 인용 "기대치" + 직접 확인 (스크래핑 0)
- SERP 실측 probe는 1인 운영엔 유지보수 함정(네이버 마크업 변경 시 파손) + egress
  차단으로 검증 불가 → 방향 전환(사용자 승인).
- 신규 `ai-citation.ts`: 키워드 **AI 브리핑 적합도**(정보성/질문형 분류) ×
  **준비도**(mate.score) = "AI 인용 기대치"(0~100). 둘 다 높아야 높게.
- 결과 화면 `AiCitationPanel`: 기대치 점수 + 구성 요약 + **적합 키워드별 네이버
  검색 딥링크**("AI 브리핑 직접 확인 ↗" — 사용자 브라우저에서 실측). 총점 미포함.
- 단위테스트 +17(분류 9, 기대치 8). 총 129건 통과.
