...
## 2026-06-15 — GEO/메이트 인용 준비도 시각화 강화 (Phase 57)

### 배경
메이트 선정까지 얼마나 부족한지 한눈에 보기 어렵다는 피드백. 기존 GeoHeadline은 점수+등급 텍스트만 표시.

### 변경
**`app/blog-diagnose/page.tsx` — GeoHeadline 컴포넌트**
- 4단계 등급 구간(재설계/개선필요/양호/인용준비완료) 색상 게이지바 추가
- 현재 점수 위치를 원형 인디케이터로 표시
- 메이트 기준선(75점) 마커 + "N점 부족" / "기준 달성!" 메시지
- 구간별 라벨 표시

**`app/components/diagnose/MateReadinessCard.tsx`**
- 5개 체크 항목별 가중치 기반 **개선 잠재 점수** 계산 (+N점 가능 뱃지)
- **우선 개선 항목** 요약 박스: 효과 큰 순서 상위 3개 + 모두 개선 시 예상 점수
- 메이트 기준까지 부족 점수 + 총 개선 여지 표시
