...
/** 내 블로그 — 메타 도구 드롭다운 (Phase 25). 향후 발행 트래킹 등 추가 예정. */
const MYBLOG_MENU: { href: string; label: string; description: string }[] = [
  { href: '/blog-diagnose', label: '블로그 진단', description: '카테고리 상위 % · 약점 분석' },
  { href: '/keyword-analysis', label: '즐겨찾기 키워드', description: '저장된 키워드 다시 분석' },
];
...
export default function Navbar() {
  ...
  const [myblogOpen, setMyblogOpen] = useState(false);
  ...
  const myblogRef = useRef<HTMLDivElement>(null);
  const userMenuRef = useRef<HTMLDivElement>(null);
  const closeTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const communityCloseTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const myblogCloseTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (myblogRef.current && !myblogRef.current.contains(e.target as Node)) {
        setMyblogOpen(false);
      }
  ...
  const openMyblog = () => {
    if (myblogCloseTimerRef.current) {
      clearTimeout(myblogCloseTimerRef.current);
      myblogCloseTimerRef.current = null;
    }
    setMyblogOpen(true);
  };

  const scheduleCloseMyblog = () => {
    if (myblogCloseTimerRef.current) clearTimeout(myblogCloseTimerRef.current);
    myblogCloseTimerRef.current = setTimeout(() => setMyblogOpen(false), 150);
  };

  const isCommunityActive = pathname.startsWith('/community');
  const isMyblogActive = pathname === '/blog-diagnose' || pathname.startsWith('/blog-diagnose/');
        ...
        <div className="flex justify-between items-center h-14">
          ...
          <div className="hidden md:flex md:items-center md:gap-1">
            ...
            <div
              className="relative"
              ref={myblogRef}
              onMouseEnter={openMyblog}
              onMouseLeave={scheduleCloseMyblog}
            >
              <button
                type="button"
                onClick={() => setMyblogOpen((v) => !v)}
                aria-expanded={myblogOpen}
                aria-haspopup="true"
                className={`flex items-center gap-1 px-3 py-2 text-sm font-medium relative transition-colors ${
                  isMyblogActive
                    ? 'text-ink dark:text-ink'
              ...
              >
                내 블로그
                <svg
                  className={`w-3.5 h-3.5 transition-transform ${myblogOpen ? 'rotate-180' : ''}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
                ...
                <span
                  aria-hidden
                  className={`absolute left-3 right-3 -bottom-px h-[2px] transition-colors ${
                    isMyblogActive ? 'bg-orange-500 dark:bg-orange-400' : 'bg-transparent'
                  }`}
                />
              </button>
              ...
              {myblogOpen && (
                <div
                  className="absolute top-full right-0 mt-1 w-72 bg-paper border border-rule overflow-hidden p-2"
                  role="menu"
                >
                  {MYBLOG_MENU.map((it) => {
                    const isActive = pathname === it.href;
                    return (
                      <Link
                        key={it.href}
                        href={it.href}
                        onClick={() => setMyblogOpen(false)}
                        className={`block px-3 py-2.5 text-sm transition-colors ${
                          isActive
                            ? 'bg-paper-deep text-orange-600 dark:text-orange-300'
                            : 'text-ink-muted hover:bg-paper-deep hover:text-ink'
                        }`}
                      >
                        <div className="font-medium font-display">{it.label}</div>
                        <div className={`text-[11px] mt-0.5 ${isActive ? 'text-orange-500/80 dark:text-orange-400/80' : 'text-ink-faint'}`}>
                          {it.description}
                        </div>
                      </Link>
                    );
        ...
        <div className="md:hidden border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 max-h-[calc(100vh-56px)] overflow-y-auto">
          <div className="px-3 py-3 space-y-4">
            ...
            <div className="pt-2 border-t border-slate-200 dark:border-slate-700 space-y-1">
              <div className="px-4">
                <span className="text-xs font-semibold text-slate-500 dark:text-slate-400">내 블로그</span>
              </div>
              {MYBLOG_MENU.map((it) => {
                const isActive = pathname === it.href;
                return (
