...
export default function EmptyState({ icon, title, description, action, variant, hints }: EmptyStateProps) {
  ...
  return (
    <div className="bg-white dark:bg-zinc-800/80 rounded-2xl border border-zinc-200 dark:border-zinc-700 p-8 sm:p-10 text-center">
      <div className="mx-auto mb-4 flex justify-center">{illustration}</div>
      <p className="text-zinc-700 dark:text-zinc-200 font-semibold text-base">{title}</p>
      {description && <p className="mt-1.5 text-sm text-zinc-500 dark:text-zinc-400 max-w-md mx-auto">{description}</p>}
...
function DefaultIllustration({ variant }: { variant?: 'swap' | 'tips' | 'companions' }) {
  ...
  const map: Record<string, { emoji: string; gradient: string }> = {
    ...
    tips:       { emoji: '💡', gradient: 'from-orange-100 to-teal-100 dark:from-orange-950/40 dark:to-teal-950/40' },
    companions: { emoji: '🚶‍♂️', gradient: 'from-rose-100 to-orange-100 dark:from-rose-950/40 dark:to-orange-950/40' },
  };
  const cfg = (variant && map[variant]) || { emoji: '✨', gradient: 'from-zinc-100 to-zinc-200 dark:from-zinc-700 dark:to-zinc-800' };
