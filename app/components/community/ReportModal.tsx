...
export default function ReportModal({ open, targetType, targetId, onClose }: ReportModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={onClose}>
      ...
      >
        <div>
          <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">게시글 신고</h3>
            ...
            {REPORT_REASONS.map((r) => (
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  {r.help && <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">{r.help}</div>}
                </div>
              </label>
            ))}
          </div>
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
            상세 설명 <span className="text-[11px] font-normal text-slate-500">{detail.length}/500 (선택)</span>
          </label>
          <textarea
            value={detail}
