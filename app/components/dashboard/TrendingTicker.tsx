...
export default function TrendingTicker({
  ...
  return (
    <section className="rounded-md border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
      <header className="flex items-center justify-between p-4 sm:p-5 border-b border-slate-100 dark:border-zinc-800">
        <div>
        ...
        {loading ? (
          <div className="flex gap-2 overflow-hidden">
            {Array.from({ length: 6 }).map((_, i) => (
              <div
                key={i}
                className="h-9 w-28 rounded-full bg-slate-100 dark:bg-zinc-800 animate-pulse flex-shrink-0"
              />
            ))}
          </div>
        ) : error ? (
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1 scrollbar-thin">
            {items.map((it) => (
              <Link
                ...
                className="group flex-shrink-0 inline-flex items-center gap-2 px-3 py-2 rounded-full border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 hover:border-blue-300 dark:hover:border-blue-700 hover:bg-blue-50 dark:hover:bg-blue-950/30 transition-colors"
              >
                <span className="text-[11px] tabular font-semibold text-slate-400 group-hover:text-blue-500 dark:group-hover:text-blue-400">
                ...
                {it.totalCount > 0 && (
                  <span className="text-[11px] tabular text-slate-500 dark:text-slate-400 whitespace-nowrap">
