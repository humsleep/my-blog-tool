      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white dark:bg-zinc-800/80 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm mb-6 overflow-hidden">
          ...
          <div className="px-5 py-4">
            ...
            <div className="flex gap-2 overflow-x-auto scrollbar-hide -mx-1 px-1 pb-1 [mask-image:linear-gradient(to_right,#000_calc(100%_-_28px),transparent)] [-webkit-mask-image:linear-gradient(to_right,#000_calc(100%_-_28px),transparent)]">
              {CATEGORIES.map((cat) => (
                <button
                  key={cat.value}
                  onClick={() => setCategory(cat.value)}
                  className={`flex-shrink-0 inline-flex items-center whitespace-nowrap min-h-[44px] px-4 rounded-full text-sm font-medium transition-all duration-150 border ${
                    category === cat.value
        ...
        {data && data.keywords.length > 0 && (
          <div className="mb-4 bg-white dark:bg-[#161618] rounded-xl border border-zinc-200 dark:border-[#27272a] p-5 shadow-sm">
            <div className="flex items-baseline justify-between mb-3">
              <h2 className="font-semibold text-zinc-900 dark:text-zinc-100">
                TOP {Math.min(10, data.keywords.length)} 한눈에
              </h2>
              <span className="text-xs text-zinc-500 dark:text-zinc-400">검색량 비례 막대</span>
            </div>
            <HorizontalBarList
