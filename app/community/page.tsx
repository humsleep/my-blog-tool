    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20">
        ...
        <header className="mb-14 max-w-4xl">
          ...
          <h1 className="font-display text-[2.5rem] sm:text-[4rem] lg:text-[5rem] leading-[0.95] tracking-tight text-ink mb-5">
            블로거들이<br />
            <span className="italic text-ink-muted">모이는 곳</span>
          </h1>
          <p className="font-display italic text-lg sm:text-xl text-ink-muted leading-[1.6] max-w-[58ch]">
          ...
          {MENUS.map((menu) => (
            ...
            >
              <div className="flex items-baseline justify-between mb-6">
                <span className="font-display italic text-5xl text-ink-faint group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors leading-none">
                  {menu.num}
                </span>
              ...
              <h2 className="font-display text-2xl sm:text-3xl font-semibold text-ink mb-3 leading-tight group-hover:text-orange-600 dark:group-hover:text-orange-400 transition-colors">
                {menu.title}
              </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-10">
          {[
            ...
            <div key={item.title} className={i < 2 ? 'sm:pr-8 sm:border-r sm:border-rule-soft' : ''}>
              <div className="font-display italic text-orange-600 dark:text-orange-400 text-2xl mb-3">{`§ ${i + 1}`}</div>
