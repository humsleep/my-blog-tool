...
export default function CompanionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return <div className="min-h-[60vh] flex items-center justify-center text-zinc-500">불러오는 중...</div>;

  if (error || !post) {
    return (
      <div className="bg-zinc-50 dark:bg-zinc-950 min-h-screen py-10">
        <div className="max-w-md mx-auto px-4 text-center">
          <p className="text-zinc-700 dark:text-zinc-200 font-medium">{error || '모집글을 찾을 수 없습니다.'}</p>
    ...
    <div className="bg-zinc-50 dark:bg-zinc-950 min-h-screen pt-6 pb-10">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <article className="bg-white dark:bg-zinc-800/80 rounded-2xl border border-zinc-200 dark:border-zinc-700 p-5 sm:p-7 shadow-sm">
          ...
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100 mb-4 break-words">{post.title}</h1>
          ...
          <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm mb-5 pb-5 border-b border-zinc-100 dark:border-zinc-700">
            {post.brand_name && (
              <>
                <dt className="text-zinc-500 dark:text-zinc-400">브랜드/매장</dt>
                <dd className="text-zinc-700 dark:text-zinc-200">{post.brand_name}</dd>
              </>
            )}
            <dt className="text-zinc-500 dark:text-zinc-400">지역</dt>
            <dd className="text-zinc-700 dark:text-zinc-200">{formatFullRegion(post.region, post.region_city)}</dd>
            <dt className="text-zinc-500 dark:text-zinc-400">방문 날짜</dt>
            <dd className="text-zinc-700 dark:text-zinc-200">
              {post.visit_date} ({dow}){post.visit_time_slot ? ` · ${post.visit_time_slot}` : ''}
            </dd>
            <dt className="text-zinc-500 dark:text-zinc-400">모집 인원</dt>
            <dd className="text-zinc-700 dark:text-zinc-200">{post.participants}명</dd>
          </dl>

          <div className="mb-5">
            <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300 mb-2">상세 내용</h2>
            <p className="text-sm text-zinc-700 dark:text-zinc-200 whitespace-pre-wrap break-words leading-relaxed">
              {post.message}
            </p>
          </div>
          ...
          <div className="mt-6 pt-5 border-t border-zinc-100 dark:border-zinc-700 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            {isMine ? (
              <>
                <div className="flex items-center gap-2">
                  ...
                  <select
                    ...
                    className="px-3 py-1.5 text-xs border border-zinc-200 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100"
                  >
