  ...
  return (
    <div className="bg-zinc-50 dark:bg-zinc-950 min-h-screen pt-6 pb-10">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="sticky top-14 z-30 -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8 py-2 bg-zinc-50/90 dark:bg-zinc-950/90 backdrop-blur-md mb-4">
          <div className="bg-white dark:bg-zinc-800/80 rounded-2xl border border-zinc-200 dark:border-zinc-700 p-3 shadow-sm">
            ...
            <div className="hidden sm:flex items-center gap-2 whitespace-nowrap overflow-x-auto scrollbar-hide">
              ...
              <label className="flex-shrink-0 inline-flex items-center gap-2 text-sm text-zinc-600 dark:text-zinc-300 cursor-pointer pl-3 border-zinc-200 dark:border-zinc-700">
                <input
                  type="checkbox"
                  checked={openOnly}
            ...
            <button
              ...
              className="sm:hidden w-full inline-flex items-center justify-between min-h-[44px] px-3 rounded-lg border border-zinc-200 dark:border-zinc-600 bg-zinc-50 dark:bg-zinc-700 text-sm font-medium text-zinc-700 dark:text-zinc-200"
            >
              ...
              <span className="text-xs text-zinc-500 dark:text-zinc-400 truncate max-w-[60%] text-right">
                {region ? `${region}${regionCity ? ' ' + regionCity : ''}` : '전체 지역'}
                {openOnly ? ' · 모집중' : ''}
              </span>
            </button>
          </div>
        ...
        {/* 모바일 필터 시트 — 지역 + 시·군·구 + 모집중 */}
        <FilterBottomSheet open={filterOpen} onClose={() => setFilterOpen(false)}>
          <div>
            <div className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 mb-2">지역</div>
            <select
              value={region ?? ''}
              onChange={(e) => { setRegion(e.target.value || null); setRegionCity(null); }}
              className="w-full px-3 py-2.5 text-base border border-zinc-200 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-orange-500"
            >
              <option value="">전체</option>
              {REGIONS.map((r) => <option key={r} value={r}>{r}</option>)}
            </select>
          ...
          {region && getCities(region).length > 0 && (
            <div>
              <div className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 mb-2">시·군·구</div>
              <select
                value={regionCity ?? ''}
                onChange={(e) => setRegionCity(e.target.value || null)}
                className="w-full px-3 py-2.5 text-base border border-zinc-200 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-orange-500"
          ...
          <label className="inline-flex items-center gap-2 text-sm text-zinc-700 dark:text-zinc-200 cursor-pointer min-h-[44px]">
            <input
              type="checkbox"
              checked={openOnly}
              onChange={(e) => setOpenOnly(e.target.checked)}
