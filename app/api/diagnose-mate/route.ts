/**
 * GET /api/diagnose-mate?blogId=<id>&category=<DiagnoseCategory>
 *
 *  사용자 RSS 12편에서 네이버 메이트(AI) 인용 준비도를 휴리스틱 분석.
 *  AI API 호출 없음. 외부 호출은 사용자 본인 RSS 1회.
 *  진단 결과 페이지에서 lazy fetch.
 */
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const report = analyzeMateReadiness(items, seed?.keywords ?? []);
    ...
    console.error('diagnose-mate error:', err);
