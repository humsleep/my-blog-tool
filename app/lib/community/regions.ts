...
/** 시·도별 시·군·구 (체험단 동행 지역 세분화) */
export const REGION_CITIES: Record<string, readonly string[]> = {
  '서울': [
...
export function getCities(region: string): readonly string[] {
  return REGION_CITIES[region] ?? [];
}

export function formatFullRegion(region: string, regionCity: string | null | undefined): string {
  if (regionCity && regionCity.trim()) return `${region} ${regionCity}`;
  return region;
}
