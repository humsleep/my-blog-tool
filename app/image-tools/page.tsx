  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-950 min-h-screen py-4 sm:py-6 md:py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* SEO 가이드 섹션 */}
        <GuideSection
          title="블로그 이미지 최적화 완벽 가이드 — 클릭률을 높이는 이미지 전략"
          items={[
            {
              title: '블로그 이미지가 SEO와 독자 경험에 미치는 영향',
              content: `블로그 포스팅에서 이미지는 단순한 장식이 아닙니다. 네이버와 구글은 이미지가 풍부하게 포함된 포스팅을 '독자에게 더 가치 있는 콘텐츠'로 평가합니다. 또한 적절한 이미지는 독자의 체류 시간을 늘리고 이탈률을 낮추어 블로그 지수 향상에도 직접적인 영향을 미칩니다.

반면 너무 크거나 최적화되지 않은 이미지는 페이지 로딩 속도를 저하시켜 오히려 마이너스 요인이 됩니다. 이미지 최적화는 화질을 유지하면서 파일 크기를 줄이고, 플랫폼에 맞는 적절한 크기와 비율로 조정하는 것이 핵심입니다.`,
            },
            {
              title: '플랫폼별 권장 이미지 크기와 비율',
              content: `네이버 블로그:
· 대표(썸네일) 이미지: 가로 1:1 또는 3:2 비율, 최소 300×300px 이상
· 본문 이미지: 최대 너비 900px, 고화질(72~96dpi) 권장
· 파일 크기: 장당 2MB 이하를 권장

티스토리:
· 대표 이미지: 1280×720px(16:9) 또는 800×600px(4:3)
· 너무 작은 이미지는 썸네일 생성 시 화질이 저하됩니다.

SNS 공유용:
· 카카오스토리/페이스북: 1200×630px (1.91:1)
· 인스타그램: 1080×1080px (1:1)
· 네이버 포스트: 800×600px 이상

이미지 편집 도구의 '리사이즈' 기능으로 업로드 전 미리 조정하세요.`,
            },
            {
              title: '모자이크 처리 — 법적 의무이자 독자 신뢰의 기본',
              content: `블로그에 타인이 등장하는 사진을 올릴 때는 반드시 개인정보 보호를 고려해야 합니다. 얼굴, 이름표, 자동차 번호판, 주소, 전화번호, 계좌번호 등 개인을 식별할 수 있는 정보는 모두 모자이크 또는 블러 처리가 필요합니다.

이는 단순한 예의의 문제가 아니라 개인정보보호법상 의무이기도 합니다. 특히 어린이 사진은 더욱 주의가 필요하며, 실내/시설 사진에도 타인이 우연히 포함된 경우 처리하는 것이 안전합니다.

모자이크 강도: 너무 약하면 원본이 보이고, 너무 강하면 이미지 맥락이 사라집니다. 대상이 완전히 식별 불가능한 수준으로 처리하되, 이미지 전체의 자연스러움을 유지하세요.`,
            },
            {
              title: '이미지 편집으로 블로그 브랜드 일관성 만들기',
              content: `성공한 블로거들의 공통점 중 하나는 '시각적 일관성'입니다. 모든 포스팅의 이미지가 비슷한 밝기, 색감, 구도를 가지면 블로그 전체가 전문 미디어처럼 보입니다.

밝기/대비 조정: 어두운 실내 사진은 밝기를 올리고 대비를 높여 선명하게 만드세요.
색온도: 음식 사진은 따뜻한 색감(노란빛), IT 리뷰는 차가운 색감(파란빛)이 어울립니다.
크롭(자르기): 핵심 피사체를 중앙에 배치하고 불필요한 여백을 제거하면 집중도가 높아집니다.

이미지 편집 도구에서 같은 설정값을 반복 적용하여 일관된 톤앤매너를 유지하세요.`,
            },
          ]}
        />
      </div>
